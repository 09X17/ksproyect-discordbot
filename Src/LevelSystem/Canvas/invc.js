...
export default class InventoryCardGenerator {
    ...
    async generate(inventoryData) {
        ...
        // Tarjeta de perfil estilo redes sociales
        await this.drawProfileCard(ctx, {
            username,
            avatarURL,
            description: description || `Nivel ${level} • ${this.formatNumber(xp)} XP`,
            followers: followers || stats.messages || 0,
            posts: posts || lootboxes.length || 0,
            verified: verified || level >= 50,
            cardStyle: 'dark' // o 'light' para la variante clara
        });
    ...
    async drawLootboxInventory(ctx, lootboxes) {
        ...
        // Mostrar hasta 6 cajas (2x3 grid)
        const visibleBoxes = lootboxes.slice(0, cols * rows);

        for (let i = 0; i < visibleBoxes.length; i++) {
            const box = visibleBoxes[i];

            const boxDef = boxTypes[box.boxType] || boxTypes[box.itemName] || {
                name: box.boxType || box.itemName || 'Caja Desconocida',
        ...
        if (lootboxes.length > cols * rows) {
            ctx.font = '16px "Poppins"';
            ctx.fillStyle = this.theme.textDim;
            ctx.textAlign = 'center';
            ctx.fillText(
                `+${lootboxes.length - (cols * rows)} items más...`,
                panelX + panelWidth / 2,
    ...
    getRarityFromBoxType(boxName) {
        if (!boxName) return 'common';

        const nameLower = boxName.toLowerCase();
        if (nameLower.includes('legendaria') || nameLower.includes('legendary')) return 'legendary';
        if (nameLower.includes('épica') || nameLower.includes('epic')) return 'epic';
        if (nameLower.includes('rara') || nameLower.includes('rare')) return 'rare';
        if (nameLower.includes('poco común') || nameLower.includes('uncommon')) return 'uncommon';
        if (nameLower.includes('mítica') || nameLower.includes('mythic')) return 'mythic';
        if (nameLower.includes('divina') || nameLower.includes('divine')) return 'divine';
        return 'common';
    }
